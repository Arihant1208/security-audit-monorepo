...
/** Mapping from layer ID to filename */
...
export function registerChecklistTools(server: McpServer): void {
  server.tool(
    "list-checklists",
    "List all available security audit checklists by layer. Returns checklist IDs and names without revealing content.",
    {},
    async () => {
  ...
  server.tool(
    "get-checklist",
    "Get the full security audit checklist for a specific layer. Contains actionable check items with PASS/FAIL/N/A/PARTIAL status options.",
    { layer: z.string().describe("Checklist layer ID, e.g. 'application-security', 'identity-access', 'api-security'") },
    async ({ layer }) => {
      ...
      if (!file) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Unknown layer "${layer}". Valid layers: ${valid}`,
            },
          ],
          isError: true,
        };
