...
export function registerMethodologyTools(server: McpServer): void {
  server.tool(
    "get-methodology",
    "Get the methodology instructions for a specific audit phase (1-7). Phase 1: System Discovery, Phase 2: Threat Modeling, Phase 3: Attack Surface Mapping, Phase 4: Layered Security Audit, Phase 5: Vulnerability Identification, Phase 6: Risk Scoring, Phase 7: Remediation Planning.",
    {
      phase: z
        .number()
        .int()
        .min(1)
        .max(7)
        .describe("Audit phase number (1-7)"),
    },
    async ({ phase }) => {
